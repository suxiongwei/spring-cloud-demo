...
@RocketMQMessageListener(topic = "${demo.rocketmq.topics.order-events}",
        consumerGroup = "${demo.rocketmq.consumer-groups.basic-marketing}",
        selectorExpression = "ORDER_CREATED || ORDER_REPLAY",
        consumeMode = ConsumeMode.CONCURRENTLY,
        messageModel = MessageModel.CLUSTERING)
public class BasicMarketingListener implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt messageExt) {
        ...
        runtimeState.recordBasicDelivery(message.getMessageKey(), properties.getConsumerGroups().getBasicMarketing());
    }
}
