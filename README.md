...
在实际的企业级开发中，除了基础的流控、熔断、热点规则外，还需要更精细化的流量治理策略。本项目中的 `service-order` 模块实现了多种企业级限流策略：

### 接口级别限流

针对单个接口设置 QPS 或并发线程数限制，适用于秒杀接口等需要严格控制访问频率的场景。

### 热点参数限流

根据请求参数值进行精细化流量控制，适用于商品 ID、用户 ID 等热点数据的访问控制。

### 系统自适应保护

基于系统负载自动调节流量，防止系统过载导致雪崩效应。

### 关联限流

一个接口的流量受另一个接口流量影响，如写操作优先于读操作的场景。

### 链路限流

同一资源在不同调用链路中采用不同限流策略，实现精细化控制。

### 熔断降级

结合 Sentinel 的熔断器功能实现服务降级，提高系统可用性。

### 黑白名单控制

基于来源 IP 或用户标识的访问控制，保护系统安全。

详细配置和使用方法请参考：
- [sentinel-nacos-config.md](services/service-order/sentinel-nacos-config.md) - 基础配置说明
- [sentinel-enterprise-strategies.md](services/service-order/sentinel-enterprise-strategies.md) - 企业级策略详细说明
- [nacos-sentinel-rules-examples.json](services/service-order/nacos-sentinel-rules-examples.json) - Nacos 配置示例（带注释说明）
- [nacos-sentinel-rules-examples-deploy.json](services/service-order/nacos-sentinel-rules-examples-deploy.json) - Nacos 配置示例（纯配置，可直接部署）
